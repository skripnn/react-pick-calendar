...
function Calendar(props) {
  ...
  var weeksOffset = 15; // количество недель за пределами видимого блока в каждую сторону

  var scrollOffset = (0, _weekWidth["default"])(weeksOffset); // значение scrollLeft, позволяющее скрыть weeksOffset за пределы блока

  var _useState = (0, _react.useState)({
    weeks: /*#__PURE__*/_react["default"].createElement("span", {
      style: {
  ...
  var _useState3 = (0, _react.useState)({
    ...
    daysPick: (0, _sortSet["default"])(props.init ? props.init.daysPick : [])
      ...
      setContent = _useState4[1]; // eslint-disable-next-line


  (0, _react.useEffect)(firstRender, []); // eslint-disable-next-line
  ...
  function getWeeks(prevWeeks) {
    ...
    var weeksCount = (0, _weeksCounter["default"])(ref.current.getBoundingClientRect().width); // сколько недель влезает в блок

    var startDate = props.startDate ? (0, _date.newDate)(props.startDate).monday() : null; // левая граница

    var endDate = props.endDate ? (0, _date.newDate)(props.endDate).monday().offsetDays(7) : null; // правая граница
    ...
    if (!prevWeeks) leftDate.offsetWeeks(-weeksOffset);else if (ref.current.scrollLeft === 0) leftDate.offsetWeeks(-weeksOffset); // влево
    ...
    if (startDate && leftDate < startDate) leftDate = (0, _date.newDate)(startDate); // если стартовая дата раньше левой границы - сдвигаем стартовую дату до границы
    ...
    if (endDate) {
      var rightDate = (0, _date.newDate)(leftDate).offsetWeeks(weeksCount); // вычисляем конечную дату

      if (endDate < rightDate) {
        // если она дальше границы
        weeksCount = endDate.getDiffWeeks(leftDate); // считаем новую длину календаря
        // 4.5 - проверка на наличие стартовой даты

        if (startDate) {
          var leftDateFromEnd = (0, _date.newDate)(endDate).offsetWeeks(-weeksCount); // вычисляем стартовую дату от конечной

          if (leftDateFromEnd < startDate) {
            // если она раньше левой границы
            leftDateFromEnd = (0, _date.newDate)(startDate); // меняем её
  ...
  function getTexts() {
    ...
    var tempMonth = {};
    var textWidth = 0;
    var mainWidth = ref.current.getBoundingClientRect().width;
    ...
    for (var i = 0; i < wCount; i++) {
      ...
      if (textWidth < mainWidth) {
        ...
        if (month !== tempMonth.month) {
          if (tempMonth.width) {
            months.push( /*#__PURE__*/_react["default"].createElement(_MonthText["default"], _extends({
        ...
        tempMonth.width += _width; // год

        var year = date.getFullYear();

        if (year !== tempYear.year) {
          if (tempYear.width) {
            years.push( /*#__PURE__*/_react["default"].createElement(_YearText["default"], _extends({
              key: tempYear.year
            }, tempYear)));
          }
        ...
        tempYear.width += _width; // счетчик ширины блока с текстом
    ...
    var width = textWidth - mainWidth; // месяц
  ...
  function week(start) {
    ...
    for (var i = 0; i < 7; i++) {
      ...
      if (props.startDate && fDate < props.startDate || props.endDate && fDate > props.endDate) daysList.push( /*#__PURE__*/_react["default"].createElement("div", {
        className: 'calendar-day hidden',
        key: fDate
      }));else daysList.push( /*#__PURE__*/_react["default"].createElement(_Day["default"], _extends({
        date: date,
        key: fDate
      }, day, {
        onClick: onDayClick
  ...
  }, /*#__PURE__*/_react["default"].createElement(_ButtonScroll["default"], {
    onClick: reset
  }), /*#__PURE__*/_react["default"].createElement(_DaysNames["default"], null)), /*#__PURE__*/_react["default"].createElement("div", {
    className: "calendar-right"
  }, /*#__PURE__*/_react["default"].createElement(_TextLine["default"], {
